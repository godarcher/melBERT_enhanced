...
def convert_examples_to_two_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if args.use_local_context:
            ...
            # ERROR APPEARED WHERE A LIST CAME UP INSTEAD OF AN INTEGER
            # AN INTEGER, HENCE WE ADDED A PART TO CHECK FOR THIS AND CONVERT IF NEEDED
            if isinstance(tokens_b, list):
            ...
            for i, w in enumerate(tokens):
                ...
                if i > tokens_b + 1 and (w in [comma1, comma2]):
                    ...
                    break
            segment_ids = [
                2 if i >= local_start and i <= local_end else 0
